<script lang="ts">
	...
	> = {
		...
		[Step.CONN]: {
			...
			description:
				'Your device is paired, connected, and ready to code.'
		}
	}
	...
	async function connectDevice(event: Event) {
		...
		if (!token || !ssid || !password) {
			errorMessage =
				'Please complete every field before connecting.'
			return
		}
	...
	<div class="grid gap-8 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)]">
		...
		>
			...
			<CardHeader class="space-y-3">
				...
				<CardDescription class="text-slate-300">
					Follow the guided steps to give your board access to GitHub
			...
			<CardContent class="flex flex-col gap-5">
				...
				>
					<strong class="text-slate-100">Pro tip:</strong> You can
					revisit a completed step to tweak the details anytime.
				</p>
			</CardContent>
		</Card>
		...
		<Card class="h-full border border-border/70 bg-card/95 shadow-xl">
			<form class="flex h-full flex-col gap-6" onsubmit={connectDevice}>
						...
						>
							...
							<div class="space-y-2">
								...
								<p class="text-sm text-muted-foreground">
									We saved your credentials locally on the
									board. You can unplug and jump straight into
									Cappy Coding.
								</p>
							</div>
					...
					{:else if step === Step.BLE}
						...
						>
							<div class="space-y-2">
								...
								/>
								<p class="text-sm text-muted-foreground">
									Create a fine-grained token with the
									repository scopes you need.
								</p>
							</div>
						</div>
				...
				<CardFooter
					class="flex items-center justify-between gap-3 pt-0"
				>
					{#if stepIndex > 0
							&& step !== Step.CONN}
						<Button
